...
def runXGB(train_X, train_y, test_X, test_y=None, test_X2=None, seed_val=0, child=1, colsample=0.5):
    ...
    if test_y is not None:
        ...
        model = xgb.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=50, verbose_eval=200)
    else:
...
def tfidf_word(train_X,train_y,train_df,test_df):
    ...
    '''
    train_tfidf_pd = pd.DataFrame(train_tfidf.toarray())
    test_tfidf_pd = pd.DataFrame(test_tfidf.toarray())
    train_df = pd.concat([train_df,train_tfidf_pd],axis=1)
    test_df = pd.concat([test_df,test_tfidf_pd],axis=1)
...
train_X_df.to_csv("tfidf_input.csv",index=False)
...
out_df.to_csv("tfidf_xgb.csv", index=False)
